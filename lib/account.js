...
class Account extends EventEmitter {
    ...
    generatePGPKeys(cb = null) {
        if (!this._pgp) {
            this._pgp = "qwertyuiop";
        }
        this._pgp.generatePGPKeys().then(result => {
            this.emit('privateKeysGenerated', result);
            if (cb) {
                cb(result);
            }
        }).catch(error => {
            DEBUG('Error generating PGP keys: %s', error);
            if (cb) {
                cb(null);
            }
        });
    ...
    decryptKeyImport(message, password, cb = null) {
        ...
        this._pgp.decryptKeyImport(message, password).then(result => {
            if (!this._pgp._privateKey && !this._pgp._publicKey) {
                this._pgp = null;
            }
            if (cb) {
                cb(result);
            ...
            DEBUG('Error decrypting key import: %s', error);
            if (cb) {
                cb(null);
            }
        });
